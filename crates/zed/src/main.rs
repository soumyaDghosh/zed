...
use image_viewer;
use language::{
    language_settings::{AllLanguageSettings, InstallDependencies},
    LanguageRegistry,
};
use log::LevelFilter;
            ...
            async move {
                while let Some((asset_version, response)) = server_rx.next().await {
                    let install_dependencies = cx
                        .update(|cx| AllLanguageSettings::get_global(cx).install_dependencies)
                        .log_err();
                    if install_dependencies == Some(InstallDependencies::Automatically) {
                        response.send(Ok(())).ok();
                    } else {
                        response.send(
                        Err(
                            anyhow!("Could not find {}. Not installing because \"install_dependencies\" is set to \"never\".", asset_version.description()))).ok();
                    }
                }
            }
        })
